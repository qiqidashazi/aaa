...
** @return
**/
TIMER_HANDLE SetAlarm(CO_Data* d, UNS32 id, TimerCallback_t callback, TIMEVAL value, TIMEVAL period)		//period：is 0, in the init Function
{
	...
	for(row_number=0, row=timers; row_number <= last_timer_raw + 1 && row_number < MAX_NB_TIMER; row_number++, row++)
	{
		...
		{	/* just store */
			...
			elapsed_time = getElapsedTime();  // 一般为0
			...
			row->callback = callback;		// initnode
			row->d = d;						// null
			row->id = id;					//	0
			row->val = value + elapsed_time;	//val
			row->interval = period;				//0
			row->state = TIMER_ARMED;			//设置模式，在timedispatch里需要用到
			return row_number;
		}
	}
