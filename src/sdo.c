...
// 将SDO传输表的里的数据，重新加载到字典里。也就是主站配置的伺服驱动的一些字典预定义配置。
...
//#define IS_SDO_ERROR(x)	((((x)&SDO_RESET) == SDO_RESET)||(((x)&SDO_ABORTED_INTERNAL) == SDO_ABORTED_INTERNAL))
...
UNS8 getSDOlineOnUse (CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS8 *line)
{
	...
	for (i = 0 ; i < SDO_MAX_SIMULTANEOUS_TRANSFERS ; i++){
		if (	/*!IS_SDO_ERROR(d->transfers[i].state)&&*/ 
				(d->transfers[i].state != SDO_RESET) &&
...
// 根据前面得到的某个客户端的编号，再根据字典里首个客户端的位置偏移加上前面的编号就能准确的传输到 用户想传输的信息的那个客户端
// pData是八个字节
UNS8 sendSDO (CO_Data* d, UNS8 whoami, UNS8 CliServNbr, UNS8 *pData)
{
	...
	/*get the server->client cobid  581 */
	if ( whoami == SDO_SERVER )	{
		...
		/* Get the client->server cobid. 602 */
		offset = d->firstIndex->SDO_CLT;
		...
		//这是单个客户端或多个客户端 进行发送的 cob-id		cob-id应该是以600+nodeid的形式进行发送  //一定要注意字典的传输
 ...
 **/
		/* 
 		假设有一台server（1），三台client（节点2,3,4,）
		在server(1)发送数据到client(2)的时候，发送的cobid为602、如果是client(3)，发送的cobid=603、client(4)，发送的cobid=604.
		在从站的程序上，一开始注册setnode()的时候是以0x02,0x03,0x04进行注册，所以在从站程序的proceedSDO下进行识别的时候就会对比到602,603,604 -> d->objdict[offset].pSubindex[1].pObject
 		那就能确定是server发送的消息，因为服务器只能有一个的原则，所以CliServNbr永远为0！！！（getSDOlineOnUse该函数在server和client上是表示不同的意思）

		如果是服务器的话：首先根据message计算需要更新字典的索引和自索引，其次是判断 当前从站传输链路里不能有当前节点为SDO_SERVER的传输。
		然后就可以根据server向client发送消息一样。首先获取传输链路里一张空表，初始化该空表，获取数据的字节数。在给传输表添加完信息后，将数据
		更新到从机的字典里。重置当前节点的传输表。最后进行完成报文0x60的发送。 					
*/
///
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	UNS8	j;
	UNS32 *pCobId = NULL;
	UNS16 offset;		// 客户端或服务器的第一个索引
	UNS16 lastIndex;	// 客户端或服务器最后一个索引
	...
	if(offset){
		while (offset <= lastIndex) {
			...
			/* Looking for the cobid received. 601 */
			// 如果是对应的client的话，client初始化节点为 （2,602,582）
	...
	if (whoami == SDO_UNKNOWN) {
		/* Am-I client ? */
		offset = d->firstIndex->SDO_CLT;  //9
		lastIndex = d->lastIndex->SDO_CLT;  //11
		j = 0;
		if(offset){
			while (offset <= lastIndex) 
			{
				...
				/* Looking for the cobid received. 582 */
	...
	// 找出之前在传输表里正在通信的传输链路的下标， 必须是已存在的，如果是服务器的话 应该是不存在的。
	err = getSDOlineOnUse( d, CliServNbr, whoami, &line );
	...
	switch (cs) {
		...
		case 1:
			...
			if (whoami == SDO_SERVER) {
				...
				if (getSDOe(m->data[0])) { /* If SDO expedited  == 1*/
					/* nb of data to be downloaded */
					nbBytes = 4 - getSDOn2(m->data[0]);  // 服务器发送数据的字节数，一般为4,2,1byte。得到的值分别为0，2，3
					/* Storing the data in the line structure. */
					d->transfers[line].count = nbBytes;
					err = SDOtoLine(d, line, nbBytes, (*m).data + 4);	//传输表里data和offset的赋值。
					...
					/* Release of the line. */
					// client在接收完服务器的数据后，并设置好字典了，需要清除当前传输表里的规则。
					resetSDOline(d, line);
				}
				...
				if (!err)
					// 传输链路的状态必须为先前注册的状态 SDO_DOWNLOAD_IN_PROGRESS
					err = (UNS8)(d->transfers[line].state != SDO_DOWNLOAD_IN_PROGRESS);
				if (err) {
				...
				// 在sdoline初始化的时候 toggle为0， & 命令字 出来的应该是0 是相等的。
		...
		case 6: /* Command specifier for data reception - the client or server is the data consumer */
			if (whoami == SDO_SERVER) { /* Server block download */
				if (err) {
					...
					//
					//	数据第一个字节是判断字节， 数据第二个字节和第三个字节为组合字节
					//	第3个字节向左位移8位异或加上第2个字节，因为是小端存储的，用来获取字典索引。如： 60 00 12 
					//	第四个字节是自索引
					//
...
/* 在对某个客户端进行 写入网络字典的时候，需要传入客户端的节点ID，可以为2或3等
	该函数会通过sdo的起始和末端偏移量寻找出字典里相对应的客户端信息。
	返回客户端的编号。
	如：三个客户端 节点ID分别为 2，3，4。nodeID与nodeIDServer应该是相等的 -- 既找到对应的客户端
*/
UNS8 GetSDOClientFromNodeId( CO_Data* d, UNS8 nodeId )
{
	UNS8 SDOfound = 0;
 ...
 ** @param data   0x80000180 + nodeId
 ** @param Callback  回调
 ** @param endianize 0 小端
 **
 ** @return
 **/

// 在发送完数据后，进行CAN调度，处理信息在case2里会对sdo的段是否结束进行判断（getSDOlineRestBytes）。如果结束既没有多余的信息要发，
// 则停止超时定时器(3s)，设置SDO传输链路state为SDO_FINISH, 调用写入网络字典时候传入的回调函数。
// 回调函数会在两个地方进行处理：1. 定时器超时  2. can报文得到正常解析时调用
		...
		UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
	...
	/* First let's find the corresponding SDO client in our OD  */
	CliNbr = GetSDOClientFromNodeId( d, nodeId);		// 如果只是单个电机的通信就为0，如果是多个电机的通信，cliNbr=对应的client
	if(CliNbr >= 0xFE)
		return CliNbr;
	
	/* Verify that there is no SDO communication yet.   -> 遍历传输表里没有当前节点的cliNbrSDO通信 */
	err = getSDOlineOnUse(d, CliNbr, SDO_CLIENT, &line);   //这里的line的含义 类似线程的调度	判断sdo传输表里与当前的cliNbr通信是否建立，如果已经建立就报错
	if (!err) {
		MSG_ERR(0x1AC4, "SDO error : Communication yet established. with node : ", nodeId);
		return 0xFF;
	}

	/* Taking the line ... */  //获取空的传输表对象作为当前节点的通信链路， 取出空传输表的下标
...
#  endif
    ...
    if(useBlockMode) {
	    ...
	    if (count <= 4) { /* Expedited transfer */
		    buf[0] = (UNS8)((1 << 5) | ((4 - count) << 2) | 3);	//0x23 写入四字节数据的命令字  ，0x2B：两字节， 0x2F：一字节
		    for (i = 4 ; i < 8 ; i++)
...
//假设是在发送的 StartSDO——Timer 中陷入了超时状态了，这里abortCode = 超时状态。
//d->transfers[line].state 应该 =  SDO_ABORTED_INTERNAL
// return SDO_FINISHED



//该函数主要对节点ID进行遍历得到逻辑上的元素下标，通过下标及C/S模式得到传输链路的下标。根据传输链路的状态选择-- 正常结束还是重试 -- 
