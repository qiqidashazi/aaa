...
UNS32 _getODentry( CO_Data* d,
                   UNS16 wIndex,      // 0x1005
                   UNS8 bSubindex,    // 0
                   void * pDestData,  // storage
                   UNS32 * pExpectedSize, //4
                   UNS8 * pDataType,      //uint32
                   UNS8 checkAccess,      //0
                   UNS8 endianize)      //0
{ /* DO NOT USE MSG_ERR because the macro may send a PDO -> infinite
  ...
  ptrTable = (*d->scanIndexOD)(d, wIndex, &errorCode);    // 指向字典里的对应索引下的对象

  if (errorCode != OD_SUCCESSFUL)
    return errorCode;

  // 实际索引的位置不能大于字典里的子索引的数量
  ...
  if (checkAccess && (ptrTable->pSubindex[bSubindex].bAccessType & WO)) {     // 0 
                   ...
                   UNS8 endianize)
{
  ...
  //const indextable* tbl;
  //tbl = (*d->scanIndexOD)(d,wIndex,&errorCode);
  ...
  if( *pExpectedSize == 0 || *pExpectedSize <= szData )
    {
      ...
      * */
      ...
      if(dataType == visible_string && *pExpectedSize < szData)
        ((UNS8*)ptrTable->pSubindex[bSubindex].pObject)[*pExpectedSize] = 0;  // 指向字典表的指针，强制转换char*，数组下第一个元素为0
