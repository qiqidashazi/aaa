	 ...
	 switch(cob_id >> 7)
	{
		...
		case PDO4rx:
			if (d->CurrentCommunicationState.csPDO)
				proceedPDO(d,m);		// pdo 通信
			break;
		case SDOtx:
		case SDOrx:
			if (d->CurrentCommunicationState.csSDO)
				proceedSDO(d,m);		// sdo 通信
			break;
		case NODE_GUARD:
			if (d->CurrentCommunicationState.csLifeGuard)
				proceedNODE_GUARD(d,m);		//MNT节点保护	一个节点只能一个节点保护或心跳机制
			break;
		case NMT:
			if (*(d->iam_a_slave))		//从站进行操作，主站不调用此函数
			{
				proceedNMTstateChange(d,m);	//NMT模块控制
			}
            break;
#ifdef CO_ENABLE_LSS
		case LSS:
...
// switchCommunicationState这里会开启很多功能比如：心跳机制、节点保护、sdo/pdo等
UNS8 setState(CO_Data* d, e_nodeState newState)
{
	if(newState != d->nodeState){
		switch( newState ){
			...
			//此处未加break 跳出，所以是在进去初始化模式后 紧接着进入了预处理模式。开启一个任务线程
