    ...
    */
    {
      ...
      */
      if (nodeId == *d->bDeviceNodeId )
        {
          ...
          msg.len = (UNS8)0x01;
          msg.rtr = 0;
          msg.data[0] = d->nodeState; //d->nodeState在setState里进行初始化
          if (d->toggle)
            {
              msg.data[0] |= 0x80 ;
              d->toggle = 0 ;
      ...
      e_nodeState newNodeState = (e_nodeState) ((*m).data[0] & 0x7F); //这里应该是Initialisation状态值
      ...
      */
      d->nodeGuardStatus[nodeId] = *d->LifeTimeFactor;   // LifeTimeFactor = 0

      if (d->NMTable[nodeId] != newNodeState) //程序刚运行初始化时候为 unknownstate,调用回调更改当前元素为新状态Initialisation
      {
        (*d->post_SlaveStateChange)(d, nodeId, newNodeState);   //无任何操作：原本该回调函数所做的操作应该是133行的代码
        /* the slave's state receievd is stored in the NMTable */
        d->NMTable[nodeId] = newNodeState;
      }

      /* Boot-Up frame reception */
      if ( d->NMTable[nodeId] == Initialisation)  //主站使用不会进入这边
      {
          /*
      ...
      if( d->NMTable[nodeId] != Unknown_state ) {
        ...
        for( index = (UNS8)0x00; index < *d->ConsumerHeartbeatCount; index++ )
          {
            // 节点通过高位开始的第二个字节开始，也就是对第三个字节与运算，计算出节点ID
  ...
  if(*d->ProducerHeartBeatTime)
    {
      ...
      ** the node-id of this device.
      时间到了，必须立即发送心跳以生成正确的节点 ID：这是通过偏移量 1792（十进制）和该设备的节点 ID 来完成的。
      */
      UNS16 tmp = *d->bDeviceNodeId + 0x700;
      msg.cob_id = UNS16_LE(tmp);
      msg.len = (UNS8)0x01;
      msg.rtr = 0;
      msg.data[0] = d->nodeState; /* No toggle for heartbeat 没有心跳切换 !*/
      /* send the heartbeat */
...
//在当前项目里只设置了心跳机制。一个节点只能同时启动一个机制
void lifeGuardInit(CO_Data* d)
{
  //  心跳机制   消费者0x1016设置   生产者0x1017未设置
  heartbeatInit(d);
  // 节点保护机制 0x100C 100D 为设定值
  nodeguardInit(d);
}
