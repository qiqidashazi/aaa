...
UNS8 buildPDO (CO_Data * d, UNS8 numPdo, Message * pdo)
{
  ...
  pdo->cob_id = (UNS16) UNS16_LE(*(UNS32*)TPDO_com->pSubindex[1].pObject & 0x7FF);  // TPDO对应下标里的COBID
  ...
  if ((*m).rtr == NOT_A_REQUEST){ 
      ...
      if (offsetObjdict)
        while (offsetObjdict <= lastIndex){
            ...
            case state2:  
            //  假设 发送机器为第二台设备 303;
            //  报文在服务器进行组装的时候，只组装了映射的实际数据，并不包含索引和子索引等...这是在SDO里做的
            //  所以在传输到不同设备上时候，只需要根据COBID和发送数据就行
...
//  pdoNum根据 PDO的传输表来决定 01234，按顺序往下排。
...
// 发送PDO的接口，在接收到同步信息到，执行遍历TPDO表里所有的PDO，将所有的PDO都发送一遍
UNS8
_sendPDOevent (CO_Data * d, UNS8 isSyncEvent)
{
  ...
  UNS16 offsetObjdict = d->firstIndex->PDO_TRS; //  获取TPDO的传输起点和终点
  UNS16 lastIndex = d->lastIndex->PDO_TRS;
  UNS16 offsetObjdictMap = d->firstIndex->PDO_TRS_MAP;    //  TPDO的MAP的起始
...
PDOInit (CO_Data * d)
{
  ...
  UNS16 offsetObjdict = d->firstIndex->PDO_TRS;  //14
  UNS16 lastIndex = d->lastIndex->PDO_TRS;   //15
  if (offsetObjdict)
    while (offsetObjdict <= lastIndex)
    {
