...
void proceedNMTstateChange(CO_Data* d, Message *m)
{
  if( d->nodeState == Pre_operational ||
    ...
    if( ( (*m).data[1] == 0 ) || ( (*m).data[1] == *d->bDeviceNodeId ) ){   // data[0] = 模块控制的模式 ， data[1] = 节点ID
      ...
      case NMT_Reset_Node:
         if(d->NMT_Slave_Node_Reset_Callback != NULL) //初始化为NULL
      ...
      case NMT_Reset_Comunication:
         ...
         UNS8 currentNodeId = getNodeId(d);
            ...
            if(d->NMT_Slave_Communications_Reset_Callback != NULL)  //初始化为NULL
